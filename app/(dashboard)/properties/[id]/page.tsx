...
export default function ListingDetailPage({
  ...
  const { mutate: transition, isPending: transitioning } =
    useTransitionListing(id);
  ...
  const { mutate: doUploadPhotos, isPending: uploadingPhotos } =
    useUploadPhotos(id);
  ...
  const [rejectReason, setRejectReason] =
    useState<RejectionReason>("missing_document");
  const [rejectNote, setRejectNote] = useState("");
  const [titleAction, setTitleAction] = useState<
    "dispute" | "clear" | "revoke" | null
  >(null);
  const [titleReason, setTitleReason] = useState("");
  const [uploadingDocs, setUploadingDocs] = useState(false);
  const [showInquiryForm, setShowInquiryForm] = useState(false);
  const [inquiryMsg, setInquiryMsg] = useState("");
  const [inquiryType, setInquiryType] = useState<"rent" | "buy" | "general">(
    "general",
  );
  ...
  const isOwner =
    role === "PROPERTY_OWNER" || listing?.createdBy === currentUser.id;
  ...
  if (!listing)
    return (
      <div className="p-8 text-center">
        <p className="text-sm text-black/40">Listing not found.</p>
        <Link
          href="/properties"
          className="text-emerald-500 text-sm mt-3 inline-block"
        >
          ← Back
        </Link>
      </div>
  ...
  return (
    <div className="p-6 md:p-8 max-w-4xl mx-auto space-y-5">
      ...
      <div className="flex items-center gap-3">
        <Link href="/properties" className="text-black/30 hover:text-black/60">
          <ArrowLeft size={18} />
        </Link>
        <div>
          ...
          <h1 className="text-xl font-semibold text-[#0f172a] line-clamp-1">
            {listing.title}
          </h1>
        </div>
      </div>
            ...
            <div className="flex items-center gap-2 mb-1.5 flex-wrap">
              <span
                className={cn(
                  ...
                  STATUS_STYLE[listing.status] ?? "bg-gray-100 text-gray-500",
                )}
              >
                {listing.status}
              </span>
              <span
                className={cn(
                  "text-[10px] font-mono uppercase px-2 py-0.5 rounded",
                  VERIFY_STYLE[listing.verificationStatus] ??
              ...
              {listing.availabilityStatus && (
                <span className="text-[10px] font-mono uppercase bg-gray-100 text-gray-500 px-2 py-0.5 rounded">
                  {listing.availabilityStatus}
                </span>
              )}
            </div>
          ...
          <div className="flex flex-wrap gap-2">
            {ownerActions.map(({ action, label, style }) => (
              <button
                ...
                className={cn(
                  "text-xs font-semibold px-3 py-2 rounded-xl transition-colors disabled:opacity-50",
                  style,
                )}
              >
                {label}
            ...
            {isOwner &&
              (listing.status === "draft" || listing.status === "rejected") && (
                <Link
                  ...
                  className="text-xs font-semibold px-3 py-2 rounded-xl border border-gray-300 text-black/60 hover:bg-gray-50"
                >
                  Edit
                </Link>
              )}
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-5">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
          {listing.bedrooms !== undefined && (
            <span className="flex items-center gap-1.5 text-sm text-black/60">
              <Bed size={13} className="text-black/30" />
              {listing.bedrooms} beds
            </span>
          )}
          {listing.bathrooms !== undefined && (
            <span className="flex items-center gap-1.5 text-sm text-black/60">
              <Bath size={13} className="text-black/30" />
              {listing.bathrooms} baths
            </span>
          )}
          {listing.area && (
            <span className="flex items-center gap-1.5 text-sm text-black/60">
              <Maximize2 size={13} className="text-black/30" />
              {listing.area.value} {listing.area.unit}
            </span>
          )}
          {listing.yearBuilt && (
            <span className="flex items-center gap-1.5 text-sm text-black/60">
          ...
          {listing.furnishingStatus && (
            <span className="text-[10px] font-mono bg-gray-100 text-gray-500 px-2 py-0.5 rounded capitalize">
              {listing.furnishingStatus.replace("_", " ")}
            </span>
          )}
        </div>
        ...
        {listing.description && (
          <p className="text-sm text-black/55 leading-relaxed mb-3 font-light">
            {listing.description}
          </p>
        )}
        {listing.neighborhoodInfo && (
          <p className="text-xs text-black/40 mb-2">
            <span className="font-medium text-black/60">Neighbourhood:</span>{" "}
            {listing.neighborhoodInfo}
          </p>
        )}
        {listing.utilityDetails && (
          <p className="text-xs text-black/40 mb-2">
            <span className="font-medium text-black/60">Utilities:</span>{" "}
            {listing.utilityDetails}
          </p>
        )}
        {listing.amenities?.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mt-2">
            {listing.amenities.map((a) => (
              <span
                key={a}
                className="text-[10px] font-mono bg-gray-100 text-black/50 px-2 py-0.5 rounded"
              >
                {a}
              </span>
            ))}
      ...
      >
        <div className="space-y-4">
          <div>
            <label className="text-[10px] font-mono uppercase tracking-widest text-black/40 mb-1 block">
              Offer amount
            </label>
          ...
          {showInquiryForm && (
            <div className="space-y-3">
              ...
              <button
                ...
                onClick={() =>
                  doSendInquiry(
                    { listingId: id, message: inquiryMsg.trim(), inquiryType },
                    {
                      onSuccess: () => {
                        setInquiryMsg("");
      ...
      {(isOwner || isAdmin) && (
        <div className="bg-white rounded-2xl border border-gray-200 p-5">
          ...
          {isOwner && listing.photos && listing.photos.length > 1 && (
            <p className="text-[9px] text-black/30 font-mono mb-2">
          ...
          {listing.photos?.length > 0 || previewPhotos.length > 0 ? (
              ...
              {(listing.photos || []).map((photo, idx) => (
                <div
                  ...
                  className={cn(
                    ...
                    dragOverIdx === idx &&
                      dragIdx !== idx &&
                      "ring-2 ring-emerald-400 ring-offset-2 scale-[1.02]",
                  )}
                >
                  <img
                    src={photo.url}
                  ...
                  />
                  {photo.isCover && (
                    <span className="absolute top-1 left-1 bg-amber-500 text-white text-[8px] font-bold px-1.5 py-0.5 rounded">
                      COVER
                    </span>
                  )}
                  {isOwner && (
                      ...
                      <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                        {!photo.isCover && (
                          <button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              doSetCover(photo.publicId);
                            }}
                            className="bg-white/90 text-black text-[9px] font-bold px-2 py-1 rounded"
                          >
                            <Star size={10} />
                          </button>
                        )}
              ...
              {previewPhotos.map((preview, i) => (
                ...
                >
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center gap-2">
                    ...
                    />
                    <span className="text-[10px] font-bold text-white bg-black/60 px-2 py-1 rounded tracking-widest shadow-sm">
                      UPLOADING...
                    </span>
                  </div>
                </div>
            ...
            <p className="text-xs text-black/35 font-light">
              No photos yet. Upload photos to improve your listing.
            </p>
          )}
        </div>
      )}
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-5">
          ...
          {listing.verificationStatus === "unverified" && isOwner && (
            <div className="flex items-start gap-2 bg-amber-50 border border-amber-200 rounded-xl p-3 text-xs text-amber-700 mb-3">
              <AlertCircle size={13} className="shrink-0 mt-0.5" /> Upload a
              title deed to start verification before submitting for review.
            </div>
          )}
          {listing.verificationStatus === "verified" && (
            <div className="flex items-center gap-2 bg-emerald-50 border border-emerald-200 rounded-xl p-3 text-xs text-emerald-700 mb-3">
              <CheckCircle2 size={13} /> Ownership verified. Listing can be
              published.
            </div>
          )}
          {docs.length > 0 ? (
            <div className="space-y-2">
              {docs.map((doc) => (
                ...
                >
                  <div>
                    <span className="text-sm text-black/70 font-mono capitalize">
                      {doc.type.replace("_", " ")}
                    </span>
                    {doc.reviewNote && (
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="text-[10px] font-mono text-blue-500 hover:text-blue-600 px-2 py-1 rounded hover:bg-blue-50"
                    >
                      View
                    </button>
                  </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-5">
          ...
          <div className="grid grid-cols-3 sm:grid-cols-5 gap-3">
            {[
              ...
              <div
                key={label}
                className="text-center bg-gray-50 rounded-xl p-3 border border-gray-200"
              >
                <p className="text-lg font-bold text-black/80">{value}</p>
                <p className="text-[9px] font-mono uppercase text-black/35 mt-0.5">
                  {label}
                </p>
              </div>
            ))}
      ...
      {(isOwner || isAdmin) && (
        <div className="bg-white rounded-2xl border border-gray-200 p-5">
          <p className="text-[10px] font-mono uppercase tracking-widest text-black/35 mb-4 flex items-center gap-1.5">
            <ShieldCheck size={10} /> On-Chain Title
          </p>
          {titleInfo ? (
            <div className="space-y-3">
              <div
                className={cn(
                  ...
                  titleInfo.verified
                    ...
                    : "bg-gray-50 border border-gray-200 text-black/60",
                )}
              >
                {titleInfo.verified ? (
                ...
                Token #{titleInfo.tokenId} —{" "}
                {titleInfo.verified ? "Verified on-chain ✓" : "Not verified"}
              </div>
              <p className="text-[10px] font-mono text-black/40 break-all">
                Contract: {titleInfo.contractAddress}
              </p>
              {isAdmin && (
                <div className="flex flex-wrap gap-2 pt-1">
                  <button
                    type="button"
                    onClick={() => setTitleAction("dispute")}
                    className="text-xs font-medium px-3 py-1.5 rounded-lg bg-orange-50 border border-orange-200 text-orange-600 hover:bg-orange-100"
                  >
                    Dispute
                  </button>
                  <button
                    type="button"
                    onClick={() => setTitleAction("clear")}
                    className="text-xs font-medium px-3 py-1.5 rounded-lg bg-emerald-50 border border-emerald-200 text-emerald-600 hover:bg-emerald-100"
                  >
                    Clear Dispute
                  </button>
                  <button
                    type="button"
                    onClick={() => setTitleAction("revoke")}
                    className="text-xs font-medium px-3 py-1.5 rounded-lg bg-red-50 border border-red-200 text-red-600 hover:bg-red-100"
                  >
                    Revoke
                  </button>
                </div>
            ...
            <div className="space-y-3">
              ...
              <div className="flex gap-2 justify-end">
                <button
                  type="button"
                  onClick={() => setShowRejectModal(false)}
                  className="text-xs text-black/40 px-4 py-2"
                >
                  Cancel
                </button>
                <button
        ...
        <>
          ...
          <div className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-50 w-full max-w-md bg-white rounded-2xl p-6 border border-gray-200 shadow-2xl">
            ...
            <div className="flex gap-2 justify-end">
              ...
              <button
                type="button"
                disabled={!titleReason.trim() || disputing}
                onClick={() => {
