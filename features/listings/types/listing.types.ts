...
export type ListingStatus =
  ...
  | 'rejected' | 'suspended' | 'rented' | 'sold' | 'archived';

export type VerificationStatus =
  | 'unverified' | 'pending' | 'verified' | 'rejected' | 'suspended';

export type AvailabilityStatus =
  | 'available' | 'under_offer' | 'rented' | 'sold';
...
export type TransitionAction =
  | 'submit' | 'start_review' | 'request_info' | 'approve' | 'reject'
  | 'publish' | 'suspend' | 'unsuspend' | 'mark_rented' | 'mark_sold'
  | 'unmark_rented' | 'unmark_sold' | 'archive';
...
export function listingToSummary(l: Listing): PropertySummary {
  ...
  return {
    ...
    image:       coverPhoto?.url ?? '/placeholder-property.jpg',
    listingType: l.listingType,
    type:        l.category === 'residential' ? 'residential' : 'commercial',
    status:      mapStatus(l.status),
    beds:        l.bedrooms,
    baths:       l.bathrooms,
    sqft:        l.area?.unit === 'sqft' ? l.area.value : l.area ? Math.round(l.area.value * 10.764) : undefined,
