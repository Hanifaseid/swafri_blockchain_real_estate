...
export async function getListings(filters?: ListingFilters): Promise<PaginatedListings> {
  try {
    ...
    const { data } = await apiClient.get<ApiPaginatedResp<Listing>>(ENDPOINTS.LISTINGS.DISCOVER, { params });
...
export async function getMyListings(): Promise<Listing[]> {
  try {
    const { data } = await apiClient.get<ApiResp<Listing[]> | ApiPaginatedResp<Listing>>(ENDPOINTS.LISTINGS.MINE);
    if (Array.isArray((data as ApiResp<Listing[]>).data)) return (data as ApiResp<Listing[]>).data;
...
export async function getListing(id: string): Promise<Listing | null> {
  try {
    const { data } = await apiClient.get<ApiResp<Listing>>(ENDPOINTS.LISTINGS.DETAIL(id));
...
export async function createListing(input: CreateListingInput): Promise<Listing> {
  const { data } = await apiClient.post<ApiResp<Listing>>(ENDPOINTS.LISTINGS.CREATE, input);
...
export async function updateListing(id: string, input: Partial<CreateListingInput>): Promise<Listing> {
  const { data } = await apiClient.patch<ApiResp<Listing>>(ENDPOINTS.LISTINGS.UPDATE(id), input);
  if (!data.success) throw new Error(data.message);
  return data.data;
}
...
export async function transitionListing(id: string, input: TransitionInput): Promise<Listing> {
  const { data } = await apiClient.post<ApiResp<Listing>>(ENDPOINTS.LISTINGS.TRANSITION(id), input);
  if (!data.success) throw new Error(data.message);
  return data.data;
}
...
export async function getAdminListings(params?: Record<string, string | number>): Promise<PaginatedListings> {
  try {
    const { data } = await apiClient.get<ApiPaginatedResp<Listing>>(ENDPOINTS.ADMIN.LISTINGS, { params });
    return extractList(data);
  } catch {
    return { items: [], total: 0, page: 1, limit: 20 };
  }
...
export interface ListingAnalytics {
  listingId: string;
  counts: { view: number; favorite: number; inquiry: number; offer: number; rental_application: number };
...
export async function getListingAnalytics(id: string): Promise<ListingAnalytics | null> {
  try {
    const { data } = await apiClient.get<ApiResp<ListingAnalytics>>(`/listings/${id}/analytics`);
    return data.success ? data.data : null;
  } catch {
    return null;
  }
...
export async function getListingDashboard(): Promise<Record<string, unknown> | null> {
  try {
    const { data } = await apiClient.get<ApiResp<Record<string, unknown>>>(ENDPOINTS.LISTINGS.DASHBOARD);
    return data.success ? data.data : null;
  } catch {
    return null;
  }
...
export async function getListingDocuments(id: string): Promise<ListingDocument[]> {
  try {
    const { data } = await apiClient.get<ApiResp<ListingDocument[]>>(ENDPOINTS.LISTINGS.LIST_DOCS(id));
...
export async function deletePhoto(listingId: string, publicId: string): Promise<void> {
  await apiClient.delete(ENDPOINTS.LISTINGS.DELETE_PHOTO(listingId), { data: { publicId } });
}

export async function setCoverPhoto(listingId: string, publicId: string): Promise<void> {
...
export async function getListingTitle(id: string): Promise<TitleInfo | null> {
  try {
    const { data } = await apiClient.get<ApiResp<TitleInfo>>(ENDPOINTS.LISTINGS.GET_TITLE(id));
    return data.success ? data.data : null;
  } catch {
    return null;
  }
...
export async function mintTitle(id: string): Promise<Listing> {
  const { data } = await apiClient.post<ApiResp<Listing>>(ENDPOINTS.LISTINGS.MINT_TITLE(id));
  if (!data.success) throw new Error(data.message);
  return data.data;
}
...
export async function clearTitleDispute(id: string, reason: string): Promise<void> {
